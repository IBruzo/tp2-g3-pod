...
def plot_batch_size_vs_map_reduce_time_2(df, city, output_dir, query_number):
    plt.figure(figsize=(8, 6))
    for lines in df['lines'].unique():
        lines_df = df[df['lines'] == lines]
        if not lines_df.empty:
            color = next(plt.gca()._get_lines.prop_cycler)['color']
            plt.plot(lines_df['batch_size'].values, lines_df['mapreduce_time'].values, marker='o', linestyle='-', color=color, label=f'MapReduce Time 1: {lines}')
            plt.plot(lines_df['batch_size'].values, lines_df['mapreduce_time_2'].values, marker='x', linestyle='--', color=color, label=f'MapReduce Time 2: {lines}')
